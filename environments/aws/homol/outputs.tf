...
output "bastion_public_ip" {
  ...
  value       = var.create_environment ? module.bastion_host_teste[0].bastion_public_ip : "N/A (ambiente destruído)"
}
...
output "app_server_private_ip" {
  ...
  value       = var.create_environment ? module.app_environment_teste[0].app_server_private_ip : "N/A (ambiente destruído)"
}

output "db_server_private_ip" {
  description = "IP Privado do servidor de banco de dados."
  value       = var.create_environment ? module.app_environment_teste[0].db_server_private_ip : "N/A (ambiente destruído)"
}
